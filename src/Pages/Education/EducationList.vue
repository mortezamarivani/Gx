          ...
          <div class="panel-body" style="background-color:#f7f7f9">
            <div class="panel panel-warning" v-if="!ShowPanel">
              ...
              <div class="panel-heading">
                <div class="row">
                  <div class="col-md-6">
                    <h2 style="direction:ltr">Show all Education list</h2>
                  </div>
                  <div class="col-md-3">
                  ...
                  <form action="#">
                    <transition enter-active-class="animated fadeIn">
                      <div class="row FilterPanel" v-if="filterPanel">
                          ...
                          <table class="table table-hover">
                            <thead class="thead-dark">
                              <tr>
                                <th>
                                  <md-switch 
                                    v-model="EducationNameSwitch"
                                    @change="AllfieldValueSwitch=false"
                                  >EducationName</md-switch>
                                </th>
                                <th>
                                  <md-switch
                                    v-model="EducationDescSwitch"
                                    @change="AllfieldValueSwitch=false"
                                  >EducationDesc</md-switch>
                                </th>
                                <th>
                                  <md-switch
                        ...
                        <div class="col-md-10">
                          <md-field md-clearable v-if="EducationNameSwitch && !AllfieldValueSwitch">
                            <label>EducationName</label>
                            <md-input v-model="EducationName" class="inputstyle"></md-input>
                          </md-field>
                          <md-field md-clearable v-if="EducationDescSwitch && !AllfieldValueSwitch">
                            <label>EducationDesc</label>
                            <md-input v-model="EducationDesc" class="inputstyle"></md-input>
                          </md-field>
                  ...
                  <table class="table table-hover">
                    <thead class="thead-dark">
                      <tr class="row-fluid">
                       ...
                       <th scope="col" class="HeaderTableStyle">EducationName</th>
                       <th scope="col" class="HeaderTableStyle">EducationDesc</th>
                       <th scope="col" class="HeaderTableStyle">Position</th>
                    ...
                    <tbody>
                      <tr v-for="(Education, index) in FilterdEducationList" :key="index">
                        <th scope="row">{{index + 1}}</th>
                        <th>{{ Education.rankID}}</th>
                        <td>{{ Education.rankName}}</td>
                        <td>{{ Education.rankDesc}}</td>
                        <td>{{ Education.position}}</td>
                        <td>{{ GetLanguge(Education.languge)}}</td>
                        <td>{{ Education.status}}</td>
                        <td>{{ Education.institute}}</td>
                        <td>
                          <md-button
                            :to="{ name: 'Education', params: { id: Education.rankID } }"
                ...
                <nav aria-label="Page navigation">
                  <ul class="pagination">
                    ...
                    <span class="md-caption">
                      ...
                      Record:( {{currentPage * countPage - (countPage-1)}} - {{currentPage * countPage}} :{{GetAllEducationCount}} )
                    </span>
                  </ul>
                </nav>
              </div>
...
export default {
  data() {
    return {
      ...
      EducationNameSwitch: false,
      EducationDescSwitch: false,
      StatusSwitch: false,
  ...
  methods: {
    ...
    DisableFiltersObjects() {
      this.EducationNameSwitch = false;
      this.EducationDescSwitch = false;
    ...
    ChangePageID(pageid, btnClicked) {
      if (btnClicked == "previous") {
        ...
        this.$store.dispatch(
          "GetEducationByPageIdFromService",
  ...
  computed: {
    GetEducationByPageId() {
      return this.$store.getters.GetEducationByPageid;
    }, //
    ...
    GetTotalPage() {
      const Educationcount = Number(this.GetAllEducationCount);
      const pagecount = Number(this.countPage);
      const paging = Educationcount / pagecount;
    ...
    GetAllEducationCount() {
      return this.$store.getters.GetAlleducationCount;
    }, //

    FilterdEducationList() {
      ...
      if(this.GetEducationByPageId.rankID != undefined){
        return null;
      }

      return this.GetEducationByPageId.filter(Element => {
        if (
          this.EducationNameSwitch == false &&
          this.EducationDescSwitch == false &&
          this.StatusSwitch == false &&
        ...
        ) 
        {
          ...
          return (
            Element.rankName.toUpperCase().match(this.allFilterValue) ||
            Element.status
              .toString()
              .toUpperCase()
              .match(this.allFilterValue) ||
            Element.rankDesc.toUpperCase().match(this.allFilterValue) ||
          ...
          return Element;
        }else if (this.EducationName =="" && this.Institute == "" && this.EducationDesc==""
         && this.Status==""  && this.Languge==""){
           return Element;
        }


        return (
          Element.rankName.toUpperCase().match(this.GetFilterValue(this.EducationName,'str')) ||
          Element.status.toString()
              .toUpperCase()
              .match(this.GetFilterValue(this.Status,'sts')) ||
          Element.rankDesc.trim().toUpperCase().match(this.GetFilterValue(this.EducationDesc,'str'))||
  ...
  created() {
    ...
    this.$store.dispatch("GetAllEducationCountFromService");
  } //created
}; //default
</script>
/*lang="scss"*/
